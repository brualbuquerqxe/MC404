...
recursive_tree_search:
	...
	mv       t1, a0                       # Ponteiro para o nó atual
	li       t3, 1                        # Contador do nível do nó
	li       a0, 0
	jal      .loopLista                   # Inicia a busca
	ret                                   # Retorna

.loopLista:

	beqz     t1, .SemResposta                # Se o nó atual for nulo, termina

	lw       t2, 0(t1)                    # Carrega o valor do nó
	lw       t4, 4(t1)                    # Carrega o nó da esquerda
	lw       t5, 8(t1)                    # Carrega o nó da direita

	addi     sp, sp, -16                  # Cria espaço na pilha
	sw       ra, 12(sp)                   # Salva o endereço de retorno
	sw       t3, 8(sp)                    # Salva o valor do contador
	sw       t4, 4(sp)                    # Salva o nó da esquerda
	sw       t5, 0(sp)                    # Salva o nó da direita

	beq      t2, a1, .valorEncontrado     # Se for igual, achou o valor

	beqz     t4, .testaDireita            # Se o nó da esquerda for nulo, testa o da direita

.testaEsquerda:
	addi     t3, t3, 1                    # Aumenta o contador
	mv       t1, t4                       # Atualiza ponteiro do nó atual
	jal      .loopLista                   # Não é nulo: pode descer mais um nível
	bge      a0, zero, .EncontrouFim # Se achou, termina

.testaDireita:
	beqz     t5, .fimLista                # Se o nó da direita for nulo, termina

	addi     t3, t3, 1                    # Aumenta o contador
	mv       t1, t5                       # Atualiza ponteiro do nó atual
	jal      .loopLista                   # Não é nulo: pode descer mais um nível
	bge      a0, zero, .EncontrouFim # Se achou, termina

.fimLista:
	mv       t1, t4                       # Atualiza ponteiro do nó atual
	addi     sp, sp,16                    # Diminui o contador
	ret

.valorEncontrado:
	mv       a0, t3                       # Retorna o nível do nó
	j        .EncontrouFim
.SemResposta:
	li       a0, -1                       # Retorna -1 (não encontrou)
	j 	  .EncontrouFim
.EncontrouFim:
	lw       t5, 0(sp)
	lw       t4, 4(sp)
