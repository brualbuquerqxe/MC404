...
criacaoImagem:
	...
	sw       ra, 12(sp)
	sw       s7, 8(sp)
	li       s11, 0                  # Contabiliza a ordem no buffer
	mv       s7, a2
...
########################################################################################################
# BORDA!! SE ESTIVER, SERÁ PRETO!!
	li       s1, 0                   # Usar como referencia
	addi     s2, a5, -1              # Última posição na altura
	beq      t1, s1, .ifBordaLargura # Faz parte da borda largura!
	beq      t1, s2, .ifBordaLargura # Faz parte da borda largura!
	beq      t2, s1, .ifBordaAltura
	addi     s2, a6, -1              # Última posição na altura
	beq      t2, s2, .ifBordaAltura

#######################################################################################################
# Recalculando a cor! posso usar s2, s8, s9, s10, s11 (posicao buffer)

# s9 acumula a soma ponderada
li   s9, 0
...
# Verifica se passou dos limites 0 e 255
mv   s5, s9
li   s8, 0
li   s2, 255
blt  s5, s8, .menor
...
.ifBordaLargura:
	li       s5, 0                   # Define a borda como preta
	j        .continua
